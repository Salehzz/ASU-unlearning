...
def get_eval_results(eval_result_dict):
    ...
    for k, v in output_result.items():
        ...
        if 'Forget' not in k:
            ...
            # forget_efficacy
            # exclude cosine similarity for forget efficacy since it can be high even for the wrong answer.
            if 'Cosine Similarity' in k:
                continue

            if 'Entropy' not in k:  # exclude the token entropy
                # The larger the value, the worse the performance on Forget Set.
                forget_efficacy_cands.append(1.0 - v)
            else:
                # for token entropy, we want it to be higher for better forget efficacy
    ...
    output_result['Forget Efficacy'] = hmean(forget_efficacy_cands)
