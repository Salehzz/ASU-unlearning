...
from asu.utils import load_model, get_params, forward_with_cache, get_data
...
def run_rmu(
    ...
    num_batches = min(
        ...
        min([len(f) * 2 for f in forget_data_list]),
        min([len(r) * 2 for r in retain_data_list]),
    )
        ...
        with tqdm.tqdm(total=num_batches) as pbar:
            for idx in range(num_batches):
                ...
                frozen_forget_activations = forward_with_cache(
                    frozen_model, unlearn_inputs, module=updated_module, no_grad=True, attention_temp=args.attention_temp_value,
                ).to(updated_model.device)

                frozen_forget_activations = frozen_forget_activations.to(updated_forget_activations.device)
                unlearn_loss = torch.nn.functional.mse_loss(
                    updated_forget_activations, frozen_forget_activations
                )
...
def get_args():
    ...
    args.attention_temp = 10.0**float(args.attention_temp)
